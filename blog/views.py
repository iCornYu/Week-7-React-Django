...
def registerPage(request):
    ...
    if form.is_valid():
        ...
        email_message = EmailMessage(
            ...
            template, #body
            settings.EMAIL_HOST_USER, #host_email
            [email], #recipient
        )
...
def updatePost(request,post_id):
    ...
    if post.author.id != request.user.id and not request.user.is_superuser:
            messages.info(request, "You can not update another user's post!")
...
def deletePost(request, post_id):
    ...
    if post.author.id != request.user.id:
        messages.info(request, "You can not delete another user's post!")
        return redirect('blog-posts')
    post.delete() 
